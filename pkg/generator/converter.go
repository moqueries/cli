...
func (c *Converter) MethodStructs(typeSpec *dst.TypeSpec, fn Func) []dst.Decl {
	...
	var decls []dst.Decl
	decls = append(decls, c.methodStruct(typeSpec.Name.Name, prefix, paramsIdent, fn.Params))
	decls = append(decls, c.resultMgrStruct(typeSpec.Name.Name, prefix))
	decls = append(decls, c.methodStruct(typeSpec.Name.Name, prefix, resultsIdent, fn.Results))
...
func (c *Converter) FuncClosure(typeName, pkgPath string, fn Func) (funcDecl *dst.FuncDecl) {
	...
	fnLitRetStmt = &dst.ReturnStmt{
		Results: []dst.Expr{fnLitCall},
	}
	if fn.Results == nil {
		fnLitRetStmt = &dst.ExprStmt{
			X: fnLitCall,
		}
	}
...
func (c *Converter) baseMockFieldsPerFn(fields []*dst.Field, typePrefix, fieldSuffix string) []*dst.Field {
	...
	fields = append(fields, &dst.Field{
		...
		Type: &dst.MapType{
			...
			Value: &dst.StarExpr{
				X: dst.NewIdent(fmt.Sprintf("%s_%s", typePrefix, resultMgrSuffix)),
			},
		},
	})
...
func (c *Converter) resultMgrStruct(typeName, prefix string) *dst.GenDecl {
		...
		Specs: []dst.Spec{
			&dst.TypeSpec{
				...
				Type: &dst.StructType{Fields: &dst.FieldList{
					List: []*dst.Field{
						{
							Names: []*dst.Ident{dst.NewIdent(c.exportName(resultsIdent))},
							Type: &dst.ArrayType{
								Elt: &dst.StarExpr{
									X: dst.NewIdent(c.exportName(fmt.Sprintf("%s_%s", prefix, resultsIdent))),
								},
							},
						},
						{
							Names: []*dst.Ident{dst.NewIdent(c.exportName(indexIdent))},
							Type:  dst.NewIdent("uint32"),
						},
						{
							Names: []*dst.Ident{dst.NewIdent(c.exportName(anyTimesIdent))},
	...
	sType.Decs.Before = dst.NewLine
	sType.Decs.Start.Append(fmt.Sprintf("// %s manages multiple results and the state of the %s type",
		structName,
		typeName))
		...
		Specs: []dst.Spec{
			&dst.TypeSpec{
				...
				Type: &dst.StructType{Fields: &dst.FieldList{List: []*dst.Field{
					...
					{
						Names: []*dst.Ident{dst.NewIdent(c.exportName(resultsIdent))},
						Type:  &dst.StarExpr{X: dst.NewIdent(fmt.Sprintf("%s_%s", prefix, resultMgrSuffix))},
					},
					{
						Names: []*dst.Ident{dst.NewIdent(c.exportName(mockIdent))},
						Type:  &dst.StarExpr{X: dst.NewIdent(mName)},
			...
			List: []dst.Stmt{
				&dst.IfStmt{
					...
					Body: &dst.BlockStmt{List: []dst.Stmt{
						&dst.IfStmt{
							Init: &dst.AssignStmt{
								Lhs: []dst.Expr{dst.NewIdent("_"), dst.NewIdent(okIdent)},
								Tok: token.DEFINE,
								Rhs: []dst.Expr{
									&dst.IndexExpr{
							...
							Body: &dst.BlockStmt{
								List: []dst.Stmt{
									&dst.ExprStmt{
										X: &dst.CallExpr{
											Fun: &dst.SelectorExpr{
												X:   cloneSelect(mockSel, c.exportName(testingIdent)),
												Sel: dst.NewIdent("Fatalf"),
											},
											Args: []dst.Expr{
												&dst.BasicLit{
							...
							Decs: dst.IfStmtDecorations{NodeDecs: dst.NodeDecs{After: dst.EmptyLine}},
						},
						&dst.AssignStmt{
							Lhs: []dst.Expr{
								&dst.SelectorExpr{
							...
							Rhs: []dst.Expr{
								&dst.UnaryExpr{
									...
									X: &dst.CompositeLit{
										...
										Elts: []dst.Expr{
											&dst.KeyValueExpr{
												Key: dst.NewIdent(c.exportName(resultsIdent)),
												Value: &dst.CompositeLit{
													Type: &dst.ArrayType{Elt: &dst.StarExpr{
														X: dst.NewIdent(c.exportName(results)),
													}},
												},
											},
											&dst.KeyValueExpr{
												Key:   dst.NewIdent(c.exportName(indexIdent)),
												Value: &dst.BasicLit{Kind: token.INT, Value: "0"},
											},
											&dst.KeyValueExpr{
												Key:   dst.NewIdent(c.exportName(anyTimesIdent)),
				...
				&dst.ReturnStmt{Results: []dst.Expr{dst.NewIdent(recorderReceiverIdent)}},
			},
		},
		Decs: stdFuncDec(),
	}
